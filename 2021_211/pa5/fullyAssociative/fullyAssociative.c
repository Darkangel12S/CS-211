...
void accessData (mem_addr_t addr, fully_associative_fifo_cache_t* cache,
    ...
    unsigned int* eviction_count) // Also increase eviction_count if a line is evicted
{
    ...
    // E means that occupancy is full (16/16)
    if (cache->occupancy == E) 
    {
        ...
        if (cache->front != NULL)
        {
            ...
            if (cache->back == temp) { cache->back = NULL; }
    ...
    if (cache->front == NULL && cache->back == NULL) // if empty cache
    {
        cache->front = newCacheLine; 
        cache->back = newCacheLine; 
    }
    else // if there is something in the cache
    {
